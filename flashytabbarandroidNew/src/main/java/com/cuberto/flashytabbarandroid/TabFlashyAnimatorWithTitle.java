...
public class TabFlashyAnimatorWithTitle extends ViewPager.SimpleOnPageChangeListener {
    ...
    public TabFlashyAnimatorWithTitle(TabLayout tabLayout) {
    ...
    private void getTabView(int position, TabLayout.Tab tab, boolean isSelected) {
        View view = tab.getCustomView() == null ? LayoutInflater.from(tabLayout.getContext()).inflate(R.layout.custom_tab_with_title, null) : tab.getCustomView();
       ...
       // set.connect(textForeground.getId(), ConstraintSet.TOP, isSelected ? title.getId() : tabImageView.getId(), ConstraintSet.BOTTOM);
        if (isSelected) {
          /*  set.clear(tabImageView.getId(), ConstraintSet.BOTTOM);
            ...
            tabImageView.setColorFilter(Color.argb(255, 242, 90, 48));
        } else {
          /*  set.connect(tabImageView.getId(), ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM);
            ...
            tabImageView.setColorFilter(Color.argb(255, 0, 0, 0));
        }
